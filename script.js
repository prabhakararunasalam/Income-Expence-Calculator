...
function addItem() {
    // Validate inputs
    if (descriptionInput.value.trim() === "" || Number(amountInput.value) <= 0) {
...
function loadItems(entry, index) {
    ...
    createCell(entry.type === "income" ? "&#10136;" : "&#10138;", entry.type === "income" ? "green" : "red");

    // Add edit and delete buttons
    const deleteCell = createCell("&#9746;", "zoom");
    deleteCell.addEventListener("click", () => deleteItem(entry));

    const editCell = createCell("&#9998;", "zoom");
    editCell.addEventListener("click", () => editItem(entry));

    // Style row based on type
    row.style.backgroundColor = entry.type === "income" ? "lightgreen" : "lightcoral";
}
